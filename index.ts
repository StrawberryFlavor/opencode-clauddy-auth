...
import { createClauddy as createClauddyProvider, type ClauddyProviderSettings } from "./provider"
import {
  ANTHROPIC_AUTH_TOKEN_ENV,
  ANTHROPIC_BASE_URL_ENV,
  AUTH_METHOD_LABEL,
  CLAUDDY_API_KEY_ENV,
  CLAUDDY_ANTHROPIC_BASE_URL,
  CLAUDDY_GEMINI_BASE_URL,
  CODEX_BASE_URL,
  PROVIDER_ID,
  GEMINI_API_CLIENT,
...
const PACKAGE_NAME = "opencode-clauddy-auth"
...
export function createClauddy(input: ClauddyProviderSettings | PluginInput) {
  if (isPluginInput(input)) return {} as Hooks
  return createClauddyProvider(input)
}
...
const getRelayBaseUrl = () => {
  const baseUrl = readEnv(ANTHROPIC_BASE_URL_ENV)
  return baseUrl ? normalizeBaseUrl(baseUrl) : undefined
}

const getCodexBaseUrl = () => getRelayBaseUrl() ?? CODEX_BASE_URL

const getGeminiBaseUrl = () => getRelayBaseUrl() ?? CLAUDDY_GEMINI_BASE_URL

const getAnthropicBaseUrl = () => getRelayBaseUrl() ?? CLAUDDY_ANTHROPIC_BASE_URL

const getApiKeyFromAuthOrEnv = (auth: Auth) => {
  if (auth.type === "api" && auth.key?.trim()) return auth.key.trim()
  return readEnv(ANTHROPIC_AUTH_TOKEN_ENV) ?? readEnv(CLAUDDY_API_KEY_ENV)
}
...
const buildGeminiUrl = (originalUrl: string, streaming: boolean, baseUrl: string) => {
  const original = new URL(originalUrl)
...
export const ClauddyCodexAuthPlugin: Plugin = async (ctx: PluginInput) => {
  ...
  const authHook: AuthHook = {
    ...
    loader: async (getAuth: () => Promise<Auth>, _provider: Provider) => {
      ...
      const apiKey = getApiKeyFromAuthOrEnv(auth)
      if (!apiKey) return {}

      const codexBaseUrl = getCodexBaseUrl()
      ...
      return {
        ...
        fetch: async (input: Request | string | URL, init?: RequestInit) => {
          ...
          if (isCodexRequest) {
            ...
            const headers = createClauddyHeaders(requestInit, apiKey, {
              promptCacheKey: transformation?.body.prompt_cache_key,
            })

            const targetUrl = rewriteUrl(originalUrl, codexBaseUrl)
          ...
          if (isGeminiRequest) {
            const geminiUrl = buildGeminiUrl(originalUrl, isGeminiStreaming, geminiBaseUrl)
          ...
          if (isClaudeRequest) {
            const targetUrl = rewriteUrl(originalUrl, anthropicBaseUrl)
...
export default ClauddyCodexAuthPlugin
