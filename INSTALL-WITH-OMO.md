# 安装 opencode-clauddy-auth + oh-my-opencode
...
帮助用户同时安装和配置：
1. `opencode-clauddy-auth` - Clauddy 中转认证插件
2. `oh-my-opencode` - OpenCode 增强功能套件

配置完成后，用户只需要一个 Clauddy key，就能使用 oh-my-opencode 的所有功能（Sisyphus、oracle、librarian 等 agents）。
...
### Step 2: 安装 opencode-clauddy-auth
...
**合并规则：**
- 如果 `plugin` 数组已存在 → 追加 `"opencode-clauddy-auth"`（如果还没有）
- 如果 `plugin` 不存在 → 创建数组
- **绝不删除现有插件**
...
**注意：** 
- `--no-tui` 避免交互界面阻塞
- 所有认证参数设为 `no`，因为我们使用 Clauddy 作为统一认证层

---

### Step 4: 配置 oh-my-opencode 使用 Clauddy 模型
...
- 所有 agents 的 `model` 都使用 `clauddy/` 前缀
- 模型分配策略：
...
>
> 你现在拥有：
> - ✅ Clauddy 中转认证插件（支持 GPT-5.2、Claude、Gemini）
> - ✅ oh-my-opencode 增强功能（Sisyphus、oracle、librarian 等 agents）
> - ✅ 所有功能只需一个 key，无需订阅多个服务
>
> **测试命令：**
> ```bash
...
| 文件路径 | 说明 |
|---------|------|
| `~/.local/share/opencode/auth.json` | 存储 Clauddy API key |
| `~/.config/opencode/opencode.json` | 注册两个插件 |
| `~/.config/opencode/oh-my-opencode.json` | 配置 agents 使用 Clauddy 模型 |
...
- 检查 `opencode.json` 是否包含 `"opencode-clauddy-auth"`
- 重启 OpenCode
...
安装成功后，用户应该能够：
1. 运行 `opencode --model clauddy/claude-opus-4-5-20251101` 正常启动
2. 在 OpenCode 中看到所有 Clauddy 模型可选
