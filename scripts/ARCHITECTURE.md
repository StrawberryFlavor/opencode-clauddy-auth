...
```
scripts/
└── install-opencode-clauddy.js  # postinstall hook for config auto-injection
```
...
|------|-------|--------|------|
| `install-opencode-clauddy.js` | Existing opencode.json | Updated opencode.json | Auto-injects provider config and plugin entry on npm install |
...
2. Adds `clauddy` provider with all supported models
